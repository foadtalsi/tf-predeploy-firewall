...
class ScopeCache:
    ...
    __slots__ = ("_by_dir", "repo_dir")

    def __init__(self, repo_dir: str) -> None:
        self.repo_dir = repo_dir
        self._by_dir: dict[str, EvalContext | None] = {}
    ...
    def _read_dir(self, directory: str) -> dict[str, bytes]:
        ...
        try:
            abs_root = Path(self.repo_dir).resolve()
            abs_dir = (Path(self.repo_dir) / directory).resolve()
        except OSError:
            return out
        ...
        # trade worth taking.
        if abs_dir != abs_root and abs_root not in abs_dir.parents:
            return out
        ...
        for e in entries:
            if e.is_dir() or e.suffix != ".tf":
                continue
            try:
                out[str(Path(directory) / e.name)] = e.read_bytes()
            except OSError:
                continue
        return out
...
def run(
    ...
    scopes = ScopeCache(opts.repo_dir)

    for f in files:
        # Collect inline ignore directives from the head revision source.
        inline_by_file[f.path] = ignore.parse_comments(f.head_content)
        ...
        scope = scopes.for_file(f.path, f.head_content)

        try:
            head_resources = parse_file_with_context(f.path, f.head_content, scope)
            ...
            findings.append(
                Finding(
                    ...
                    resource="-",
                    message=f"could not parse file as HCL: {exc}",
                )
            )
            continue

        base_by_addr: dict[str, Resource] = {}
        if f.base_content is not None:
            ...
            try:
                for r in parse_file(f.path, f.base_content):
                    base_by_addr[r.address()] = r
        ...
        in_ = FileInput(
            ...
            head_source=f.head_content,
            base_resources=base_by_addr,
        )
        for rule in ruleset:
            findings.extend(rule.check(in_, kb))
    ...
    kept = ignore.apply(findings, inline_by_file, opts.global_ignore)
    attach_doc_urls(kept, kb)

    return Result(findings=kept, changed_attrs=changed_attrs)


def attach_doc_urls(findings: list[Finding], kb: KnowledgeBase | None) -> None:
    ...
    cache: dict[str, str] = {}
    for f in findings:
        if f.doc_url or not f.resource:
            continue
        url = cache.get(f.resource)
        if url is None:
            url = ""
            r_type, is_data, ok = type_from_address(f.resource)
            if ok:
                url = kb.doc_url(r_type, is_data)
